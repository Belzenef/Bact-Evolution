...
# | ainit | T | S/L |
data<-read.table("SimulationOutputspmut0.csv", header = FALSE, sep = ",", dec=".")
...
plot(extinction$ainit,extinction$T, col='black', pch=15, cex=c, las=1, main="Portrait de phase",
...
legend("topright",title="S/L",inset=c(-0.13,0),pch=15,col=cbind(color,c("black","#990033")),
       legend=cbind(seq(min(cohabitation$result),max(cohabitation$result),
                        length.out = length(color)),c("extinction","exclusion")),bty="n")

# | ainit | T | S/L |
data<-read.table("SimulationOutputspmut0001.csv", header = FALSE, sep = ",", dec=".")
names(data)<-c("ainit","T","result")
...
points(exclusion$ainit,exclusion$T, col="#990033", pch=15, cex=c)
points(cohabitation$ainit,cohabitation$T, col=color, pch=15, cex=c)
legend("topright",title="S/L",inset=c(-0.13,0),pch=15,col=cbind(c(color[1],color[n]),c("black","#990033")),
       legend=cbind(seq(min(cohabitation$result),max(cohabitation$result),
