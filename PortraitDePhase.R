...
# | ainit | T | S/L |
data<-read.table("SimulationOutputspmut.csv", header = FALSE, sep = ",", dec=".")
...
plot(extinction$ainit,extinction$T, col='black', pch=15, cex=c, las=1, main="Portrait de phase - pmut=0",
...
legend("topright",title="S/L",inset=c(-0.1,0),pch=15,col=cbind(c(color[1],color[n]),c("black","#990033")),
       legend=cbind(seq(min(cohabitation$result),max(cohabitation$result),
                        length.out = 2),c("extinction","exclusion")),bty="n", xpd=NA)
